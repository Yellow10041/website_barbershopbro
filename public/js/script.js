...
/* harmony import */ var lodash_fp_util__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! lodash/fp/_util */ "./node_modules/lodash/fp/_util.js");
/* harmony import */ var lodash_fp_util__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(lodash_fp_util__WEBPACK_IMPORTED_MODULE_0__);
...
$(function () {
  ...
  $('.container__filer__box').click(function () {
    if (i) {
      $('.container__filer__box__plate').css({
        "z-index": "10"
      });
      $(".container__filer__box__item").css({
        "opacity": "0"
      });
      setTimeout(function () {
        $('.container__filer__box').css({
          'height': '35px'
        });
        $(".container__filer__box__item").eq(0).css({
          "display": "block",
          "opacity": "1"
        });
  ...
  var placeItems = $(".container__map__place__item").length;
  console.log(placeItems);
  var placeParPosX = $('.container__map__inner').offset().left;
  var placeParPosY = $('.container__map__inner').offset().top;

  for (var _i = 0; _i < placeItems; _i++) {
    var placeItemX = $('.container__map__place__item:eq(' + _i + ')').offset().left - placeParPosX;
    var placeItemY = $('.container__map__place__item:eq(' + _i + ')').offset().top - placeParPosY;
    $('.container__map__point__item:eq(' + _i + ')').css({
      'top': placeItemY - 60,
      'left': placeItemX + 10
    });
    $('.container__map__employercard__item:eq(' + _i + ')').css({
      'top': placeItemY - 330,
      'left': placeItemX + 50
    });
  ...
  $('.container__map__point__item').click(function () {
    $('.container__map__point__item-color').removeClass('container__map__point__item-coloractive');
    $('.container__map__employercard__item').removeClass('container__map__employercard__item-active');
    var pointItemIndex = $(this).index() - 1;
  ...
  function timeminutsPos() {
    var posX = $('.container__timetable__time__item-active').offset().left;
    var itemWidth = $('.container__timetable__time__item-active').width();
    var itemMinWidth = $('.container__timetable__timeminuts').width();
    var posParX = $('.container__timetable__inner').offset().left;
    console.log(itemMinWidth);
    $('.container__timetable__timeminuts').css({
      'left': posX - posParX - itemMinWidth / 2 + itemWidth / 2
    });
  }
  ...
  $('.container__timetable__time__item').click(function () {
    ...
    var hourNumStart = Number($('.container__timetable__timeminuts__item').children('span:first').text());
    $('.container__timetable__timeminuts__item:first').data('time', 10);
    var hourNumEnd = Number($(this).children('span').text());
    var difference = hourNumEnd - hourNumStart;
    var antiDifference = hourNumStart - hourNumEnd;
    console.log(hourNumStart, hourNumEnd, difference);

    if (difference > 0) {
      for (var _i2 = 1; _i2 <= difference; _i2++) {
        $('.container__timetable__timeminuts__item').children('span:first-child').text(hourNumStart + _i2);
      }
    } else {
      for (var _i3 = 1; _i3 <= antiDifference; _i3++) {
        $('.container__timetable__timeminuts__item').children('span:first-child').text(hourNumStart - _i3);
      }
    }
  ...
  $('.container__timetable__timeminuts__item').click(function () {
    ...
    $('.container__timetable__timeminuts__item').each(function () {
      ...
      var timeStart = Number($(this).children('span:first').text()) * 60 + Number($(this).children('span:last').text());
      var timeEnd = Number($(this).children('span:first').text()) * 60 + Number($(this).children('span:last').text()) + Number($('.container__services__item-active').data('time'));
  ...
  $('.container__timetable__timeminuts__item').click(function () {
    var timeStart = Number($(this).children('span').eq(0).text()) * 60 + Number($(this).children('span').eq(1).text());
    var timeService = $('.container__services__item-active').data('time');
    var timeEnd = timeStart + timeService;
  ...
  $('.container__map__employercard__button').click(function () {
    var employer = $(this).parent().parent().children('.container__map__employercard__id').text();
    var employerMob = $(this).parents('.container__card__item').children('.container__card__item__id').text();
